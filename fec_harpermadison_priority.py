"""Priority FEC enrichment for Harper-Madison donors who are unmatched."""
import sqlite3, sys, io, time, requests
...
cur.execute("""
    ...
    WHERE cf.recipient = 'Harper-Madison, Natasha N.'
      AND cf.correction != 'X'
...
print(f"Harper-Madison priority: {len(priority_donors)} unmatched donors", flush=True)
...
session.headers.update({"User-Agent": "Austin Finance Research / harpermadison-priority"})
