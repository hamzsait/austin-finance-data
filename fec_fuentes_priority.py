"""Priority FEC enrichment for Fuentes donors who are unmatched."""
...
cur.execute("""
    ...
    WHERE cf.recipient = 'Fuentes, Vanessa'
      AND cf.correction != 'X'
...
print(f"Fuentes priority: {len(priority_donors)} unmatched donors", flush=True)
...
session.headers.update({"User-Agent": "Austin Finance Research / fuentes-priority"})
