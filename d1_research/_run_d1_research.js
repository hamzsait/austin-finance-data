/* D1 candidate donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending d1batch_*.json files
 * (the 606-donor >=$100 District 1 candidate pool) with a worker pool, retries,
 * and usage-limit-aware global backoff. Same pattern and same v3 mandatory/
 * balanced instructions as travis_research/_run_production_v3.js.
 *
 * Logs per-batch token usage + cost to _d1_usage_log.jsonl.
 * Globs only d1batch_* in this directory -- does not touch travis_research/.
 */
...
const USAGE_LOG = path.join(ROOT, '_d1_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^d1batch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given across the five 2026 Austin City Council District 1 candidates: Amber Goodwin, Misael Ramos, Alexandria Anderson, Steven Brown, and Portia Riggins). These are donors to candidates in a live open-seat municipal race. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_d1_job_errors.log'),
