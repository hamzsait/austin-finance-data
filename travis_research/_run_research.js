/* Research driver: run headless `claude -p --model opus` donor-research jobs
 * over all pending donorbatch3_*.json files with a worker pool, retries, and
 * usage-limit-aware global backoff (same pattern as the extraction driver).
 */
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^donorbatch3_\d+\.json$/.test(f))
...
Write results to ${path.join(ROOT, out)} (JSON array, donor-batch output format).`;
}
...
function runJob(batchFile) {
  return new Promise(resolve => {
    const child = spawn('claude',
      ['-p', '--model', 'opus', '--allowedTools', 'Read,Write,WebSearch,WebFetch,ToolSearch'],
      { shell: true, stdio: ['pipe', 'pipe', 'pipe'] });
    ...
    child.on('close', code => {
      clearTimeout(t);
      const outPath = path.join(ROOT, batchFile.replace('.json', '_results.json'));
      if (!fs.existsSync(outPath)) return resolve(`no output (exit ${code})`);
      const bad = goodOutput(batchFile);
      if (bad) { try { fs.unlinkSync(outPath); } catch {} return resolve(bad); }
      resolve(null);
    });
  ...
  while (true) {
    ...
    await Promise.all(Array.from({ length: Math.min(POOL, pending.length) }, async () => {
      while (idx < pending.length) {
        ...
        while (true) {
          const t0 = Date.now();
          const bad = await runJob(b);
          const secs = Math.round((Date.now() - t0) / 1000);
          if (!bad) { done++; console.log(`OK ${done} ${b} [${secs}s]`); break; }
