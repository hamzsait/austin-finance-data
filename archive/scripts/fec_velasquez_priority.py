"""
Priority FEC enrichment for Velasquez donors who are unmatched.
Same shape as fec_priority_batch.py but targeted at Velasquez's donor pool.
"""
import sqlite3, sys, os, io, time, requests
...
cur.execute("""
    ...
    WHERE cf.recipient = 'Velasquez, Jose'
      AND cf.correction != 'X'
...
print(f"Velasquez priority: {len(priority_donors)} unmatched donors", flush=True)
...
session.headers.update({"User-Agent": "Austin Finance Research / velasquez-priority"})
