"""Apply D1 donor research results to the DB.

Mirrors travis_research/_apply_results.py exactly -- same columns, same
confidence gate, same dedup rule -- but globs d1batch_*_results.json in this
directory instead of donorbatch*_results.json in travis_research/. Kept as a
separate file so the Travis apply path stays untouched.

  d1batch_*_results.json -> donor_identities.resolved_industry /
...
Usage:
    python _apply_d1_results.py            # apply
    python _apply_d1_results.py --dry-run  # report only, no writes
"""
import glob
import json
import os
...
cat_counts = {}
files = sorted(glob.glob(os.path.join(ROOT, "d1batch_*_results.json")))
